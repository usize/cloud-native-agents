...
class KafkaSystemLauncher:
    def __init__(self):
        ...
        # Use cluster-provided bootstrap servers (comma-separated allowed)
        self.bootstrap_servers = os.getenv("KAFKA_BOOTSTRAP_SERVERS", "localhost:9092")
        
    def check_kafka_running(self) -> bool:
        """Check if Kafka is reachable at KAFKA_BOOTSTRAP_SERVERS."""
        try:
            import socket
            # Take the first broker from comma-separated list
            first = self.bootstrap_servers.split(',')[0].strip()
            host, port_str = first.split(':', 1)
            port = int(port_str)
    ...
    def check_topics_exist(self) -> bool:
        """Best-effort check for topics. In OpenShift, rely on cluster management or auto-create."""
        # Skipping CLI checks inside the container; assume topics are pre-created
        return True
    
    def create_topics(self):
        """Skip topic creation in container; document expectation instead."""
        print("ℹ️ Skipping topic creation inside container. Ensure required topics exist in the Kafka cluster or enable auto-create.")
        return True
    ...
    def run(self):
        ...
        if not self.check_kafka_running():
            print("❌ Kafka is not reachable at KAFKA_BOOTSTRAP_SERVERS=", self.bootstrap_servers)
            print("Please ensure the cluster broker is accessible from the pod and the value is set correctly in the ConfigMap/Deployment.")
            return
        ...
        if not self.check_topics_exist():
            ...
            print("✅ Skipping topic checks (managed externally)")
